...
if not df.empty:
    ...
    with colc2:
        ...
        # Display the chart in Streamlit
        st.markdown('<h3 class="custom-subheader">Total Premium and Number of Sales for Eden Care Intermediaries</h3>', unsafe_allow_html=True)
    ...
    monthly_product = df.groupby(['Start Month', 'Product_name'])['Total Premium'].sum().unstack().fillna(0)
    ...
    with colc1:
        ...
        # Display the Total Premium chart in Streamlit
        st.markdown('<h3 class="custom-subheader">Premium Distribution by Cover Type for Eden Care Intermediaries</h3>', unsafe_allow_html=True)
    ...
    df_grouped = df.groupby(['Client Name', 'Owner'])['Total Premium'].sum().nlargest(15).reset_index()
